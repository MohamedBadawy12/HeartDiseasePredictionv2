...
namespace Repositories
{
    public class AppointmentRepository : IAppointmentRepository
    {
        ...
        public async Task<IEnumerable<Appointment>> GetTodaysAppointments(int id)
        {
            ...
            return await _context.Appointments
                .Where(d => d.DoctorId == id && d.StartTime >= today)
                .Include(p => p.Patient)
        ...
        public async Task<IEnumerable<Appointment>> GetUpcommingAppointments(string userId)
        {
            DateTime today = DateTime.Now.Date;
            return await _context.Appointments
                .Where(d => d.Doctor.UserId == userId && d.StartTime >= today && d.Status == true)
                .Include(p => p.Patient)
                .OrderBy(d => d.StartTime)
                .ToListAsync();
        }
        ...
        public IQueryable<Appointment> FilterAppointments(AppointmentSearchVM searchModel)
        {
            ...
            if (searchModel != null)
            {
                ...
                if (!string.IsNullOrWhiteSpace(searchModel.Option))
                {
                    ...
                    {
                        result = result.Where(x => Convert.ToDateTime(x.StartTime).Year == DateTime.Now.Year && Convert.ToDateTime(x.StartTime).Month == DateTime.Now.Month);
                    }
        ...
        {
            return await _context.Appointments
                .Where(a => a.StartTime != null && a.StartTime.Value.Date == getDate.Date)
                .Include(p => p.Patient)
                .Include(d => d.Doctor)
                .ToListAsync();
        }
        ...
        {
            return await _context.Appointments.AnyAsync(a => a.StartTime == appntDate && a.DoctorId == id);
        }
        /// <summary>
